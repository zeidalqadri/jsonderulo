...
class JsonderuloExtension {
  private config: JsonderuloConfig;
  private costPanel: vscode.WebviewPanel | undefined;
  ...
  private loadConfiguration(): JsonderuloConfig {
  ...
  private registerCommands() {
    ...
    const generateSchemaCommand = vscode.commands.registerCommand(
      'jsonderulo.generateSchema',
      async () => {
    ...
    const validateJsonCommand = vscode.commands.registerCommand(
      ...
      async () => {
        ...
        try {
          ...
          const result = await this.validateJson(
    ...
    // Repair JSON
    const repairJsonCommand = vscode.commands.registerCommand(
      'jsonderulo.repairJson',
      async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
          vscode.window.showErrorMessage('No active editor');
    ...
    // Show Cost Analytics
    const showCostAnalyticsCommand = vscode.commands.registerCommand(
      'jsonderulo.showCostAnalytics',
      async () => {
  ...
  private async generateSchema(description: string): Promise<any> {
    const response = await axios.post(`${this.config.apiEndpoint}/schema/generate`, {
      description,
    ...
    if (this.config.showCostWarnings && response.data.costMetrics?.totalCost > 0.01) {
      vscode.window.showWarningMessage(
        `High cost operation: $${response.data.costMetrics.totalCost.toFixed(4)}`
      );
    }
  ...
  private async createPrompt(request: string, schema?: string): Promise<any> {
    const response = await axios.post(`${this.config.apiEndpoint}/prompt/create`, {
      request,
      schema,
      provider: this.config.defaultProvider,
      model: this.config.defaultModel,
  ...
  private async validateJson(json: string, schema: string): Promise<ValidationResult> {
    const response = await axios.post(`${this.config.apiEndpoint}/validate`, {
      json,
      schema,
    });
  ...
  private async repairJson(json: string): Promise<string | null> {
    const response = await axios.post(`${this.config.apiEndpoint}/repair`, {
      json,
    });
  ...
  private async showPromptResult(result: any) {
    const panel = vscode.window.createWebviewPanel(
      'jsonderuloPrompt',
      'Jsonderulo Prompt Result',
  ...
  private async showValidationResult(result: ValidationResult) {
    if (result.valid) {
  ...
  private async showCostAnalytics() {
    if (this.costPanel) {
      this.costPanel.reveal(vscode.ViewColumn.Two);
    } else {
      this.costPanel = vscode.window.createWebviewPanel(
        'jsonderuloCost',
        'Jsonderulo Cost Analytics',
        vscode.ViewColumn.Two,
        {
          enableScripts: true,
        }
      ...
      try {
        const response = await axios.get(`${this.config.apiEndpoint}/analytics/cost`);
  ...
  private async useTemplate(templateName: string) {
    ...
    try {
      const response = await axios.post(`${this.config.apiEndpoint}/template/use`, {
        templateName,
        variables,
      });
  ...
  private getPromptResultHtml(result: any): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  private getCostAnalyticsHtml(analytics: any): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        ...
        ${Object.entries(analytics.costByProvider).map(([provider, cost]: [string, any]) => `
          <div class="metric">${provider}: $${cost.toFixed(4)}</div>
        `).join('')}
        
        <h3>Optimization Suggestions</h3>
        ${analytics.suggestions.map((suggestion: string) => `
